    ...
    if not GITHUB_TOKEN:
        ...
        if is_all_up:
            if downtime_issue:
                ...
                # --- UPDATED: Use IST in the issue comment ---
                downtime_issue.create_comment(f"Resolved: The system came back online at {up_at_ist.strftime('%Y-%m-%d %H:%M:%S IST')}.")
            ...
            if not downtime_issue:
                ...
                # --- UPDATED: Use IST in the issue body ---
                repo.create_issue(
                    title=DOWNTIME_ISSUE_TITLE,
                    body=f"The monitor detected that the page `{failing_url}` went down at {now_ist.strftime('%Y-%m-%d %H:%M:%S IST')}."
                )
